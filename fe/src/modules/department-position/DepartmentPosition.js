...
const DepartmentPositions = () => {
  ...
  const handleSubmit = (values) => {
    const existingRecord = departmentPositions.find(
      (t) =>
        t.department.departmentCode === values.departmentCode &&
    ...
    axios.post(
      "http://localhost:8080/user-management/department-positions/add",
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          await axios.delete(
            `http://localhost:8080/user-management/department-positions/${id}`
          );

          // Update the positions state by filtering out the deleted position
          setDepartmentPositions(
            departmentPositions.filter((item) => item.departmentCode !== id)
          );
          ...
          <Form form={form} layout="vertical" onFinish={handleSubmit}>
            ...
            <Form.Item
              ...
              rules={[
                {
                  required: true,
                  message: "Vui lòng nhập mã vị trí gồm 1 kí tự",
                },
              ]}
            >
              <Input />
