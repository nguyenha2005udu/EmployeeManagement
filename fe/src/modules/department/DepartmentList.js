    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          await axios.delete(
            `http://localhost:8080/user-management/departments/${id}`
          );
    ...
    },
    {
      ...
      render: (status) => (
        <Tag color={getStatusColor(status)}>
          {status === true ? "Đang hoạt động" : "Không hoạt động"}
        </Tag>
      ),
    },
    {
              ...
              rules={[
                {
                  required: true,
                  message:
                    "Vui lòng trạng thái của phòng ban (Active/Inactive)",
                },
              ]}
            >
              <Input />
