...
const TimesheetList = () => {
  ...
  const fetchTimesheets = async (day, month, year) => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/user-management/attendances/${day}/${month}/${year}`
      );
      const timesheets = response.data.map((item) => ({
        id: item.id,
        employeeId: item.employee?.employeeId || "", // Ensure this matches the database
        checkIn: item.checkIn || "", // Ensure this matches the database
        checkOut: item.checkOut || "", // Ensure this matches the database
        date: item.date, // Ensure this matches the database
        workDuration: item.workDuration || 0, // Ensure this matches the database
        status: item.status || "Có mặt", // Ensure this matches the database
        note: item.note || "", // Ensure this matches the database
      }));

      setTimesheets(timesheets);
      setStatistics(calculateStatistics(timesheets)); // Ensure this function is updated accordingly
    } catch (error) {
  ...
  const handleSubmit = async (values) => {
    ...
    const existingRecord = timesheets.find(
      (t) =>
        t.employee.employeeId === values.employeeId && currentDate === t.date
    );
    ...
    try {
      await axios.post(
        "http://localhost:8080/user-management/attendances/add",
        employeeCheckin
      );
  ...
  const handleCheckIn = async (id) => {
    try {
      ...
      console.log(
        ...
        "abc",
        checkInTime
      ); // Thời gian check-in theo định dạng "YYYY-MM-DDTHH:mm:ss"

      await timesheetService.checkIn(id, checkInTime);

      setTimesheets((prev) =>
        prev.map((t) => (t.id === id ? { ...t, checkInTime: checkInTime } : t))
      );
  ...
  const handleCheckOut = async (id) => {
    try {
      ...
      console.log(
        ...
        "Local Time:",
        checkOutTime
      ); // Log dữ liệu check-out với giờ địa phương
              ...
              <Col span={6}>
                <Card>
                  <Statistic
                    title="Vắng mặt"
                    value={
                      employees.length - statistics.present - statistics.late
                    }
