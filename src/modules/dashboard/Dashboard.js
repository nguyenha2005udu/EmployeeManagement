...
const Dashboard = () => {
  ...
  const newEmployees = [
    { key: '1', id: 1, name: "Nguyễn Văn A", department: "IT", joinDate: "2024-03-01" },
    { key: '2', id: 2, name: "Trần Thị B", department: "HR", joinDate: "2024-03-05" },
  ];
  ...
  return (
    <div style={{ padding: '24px', position: 'relative', zIndex: 1 }}>
      ...
      <Row gutter={16} className="mb-6">
        <Col span={24}>
          <Card title="Thống Kê Chấm Công Hôm Nay" hoverable>
            <Row gutter={16}>
              <Col span={8}>
                <Statistic
                  title="Đúng Giờ"
      ...
      <Row gutter={16} className="mb-6">
        <Col span={12}>
          <Card title="Tỷ Lệ Đi Làm Theo Phòng Ban" hoverable>
            <Row gutter={[16, 16]}>
              {departments.map(dept => (
                <Col span={8} key={dept.key}>
                  ...
                  />
                  <div className="text-center mt-2">{dept.name}</div>
                </Col>
              ))}
            </Row>
          </Card>
      ...
      <Row gutter={16}>
        <Col span={24}>
          <Card title="Thống Kê Nhân Sự" hoverable className="mb-6">
            <Row gutter={16}>
              {[
                { key: 'male', title: 'Nam', value: 90, percent: 60 },
                { key: 'female', title: 'Nữ', value: 60, percent: 40 },
                { key: 'fulltime', title: 'Toàn thời gian', value: 130, percent: 87 },
                { key: 'parttime', title: 'Bán thời gian', value: 20, percent: 13 },
              ].map(item => (
                <Col span={6} key={item.key}>
                  <Statistic title={item.title} value={item.value} suffix="/150" />
